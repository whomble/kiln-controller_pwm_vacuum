...
function updateProfile(id)
{
	...
	var job_seconds = profiles[id].data.length === 0 ? 0 : parseFloat(profiles[id].data[profiles[id].data.length-1][0]);
...
function updateProfileTable()
{
	...
	html += '<tr><th style="width: 50px">#</th><th>Target Time in ' + time_scale_long+ '</th><th>Target Temperature in °'+temp_scale_display+'</th><th>Target Pressure in mbar'+'</th><th>10^x'+'</th><th>Slope in &deg;'+temp_scale_display+'/'+time_scale_slope+'</th><th></th></tr>';
	...
	for(var i=0; i<graph.profile.data.length;i++)
	{
		...
		html += '<td><input type="text" class="form-control" id="profiletable-1-'+i+'" value="'+ graph.profile.data[i][1] + '" style="width: 60px" /></td>'; //pression
		html += '<td><input type="text" class="form-control" id="profiletable-2-'+i+'" value="'+ graph.profile.data[i][2] + '" style="width: 60px" /></td>';
		html += '<td><input type="text" class="form-control" id="profiletable-3-'+i+'" value="'+ graph.profile.data[i][3] + '" style="width: 60px" /></td>'; //puissance
...
function timeProfileFormatter(val, down) {
	...
	//return Math.(rval);
	return rval; // maintenant, le temps est en flotant
}
...
function newPoint()
{
	...
	graph.profile.data.push([pointx, Math.floor((Math.random()*230)+25), 0, 0]);
	graph.plot = $.plot("#graph_container", [ graph.profile, graph.live ] , getOptions() );
	//autreColonne(graph.profile.data);
	//alert("RIEN");
	//graph.plot = $.plot("#graph_container", [ graph.profile, graph.live ] , getOptions() );
	//autreColonne(graph.profile.data);
	updateProfileTable();
...
function saveProfile()
{
	...
	var n=graph.profile.data.length;
	var data=[];
	var a;
	//console.log(n);
	for (let i=0; i<n; i++) {
		...
		for (let j=0; j<4; j++) {
			if (j==0) {
				//a.push(document.getElementById("profiletable-"+j+"-"+i).value * 3600);
				a.push(document.getElementById("profiletable-"+j+"-"+i).value * 3600);
			} else {
				a.push(document.getElementById("profiletable-"+j+"-"+i).value);
			}
			//a.push(document.getElementById("profiletable-"+j+"-"+i).value);
		}
		//console.log(a);
			...
			ws_status.onmessage = function(e)
			{
				...
				if(state!="EDIT")
				{
					...
					if(state=="RUNNING")
					{
						...
						left = parseFloat(x.totaltime-x.runtime);
